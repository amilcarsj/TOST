...
def interpolate(trip_df):
  ...
  '''
  interpolated_trip_df = trip_df.copy()
  interpolated_trip_df['interpolated'] = False

  prev_timestamp = None
  for index in trip_df.index:
    row = trip_df.loc[index]
    ...
    if prev_timestamp != None:
      ...
      if time_diff > interval_between_interpolations_in_sec * 2: # 6 min
        ...
        times = get_times(trip_df.loc[index-1], row)
  ...
  return interpolated_trip_df.sort_values(by=['timestamp_in_seconds']).reset_index(drop=True)
...
def get_xytvv(trip_df, index_point_a, index_point_b):
  row_a = trip_df.loc[index_point_a]
  row_b = trip_df.loc[index_point_b]
  point_a_xy_speeds = calc_speed(trip_df,index_point_a - 1, index_point_a) # calculate x and y speed on the point right before the gap
  point_b_xy_speeds = calc_speed(trip_df, index_point_b, index_point_b + 1) # calculate speed on the point after the gap
...
# the list of rows
def calc_speed(trip_df, index_a, index_b):
    is_out_limits = index_a < 0 or index_b >= len(trip_df) # this happens when measuring the speed on the first or last point
    if (is_out_limits):
      return [0, 0]
    row_a = trip_df.loc[index_a]
    row_b = trip_df.loc[index_b]
